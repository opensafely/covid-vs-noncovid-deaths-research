...
*CHANGE THE OUTCOME BELOW TO LAST IF BRINGING IN MORE COLS
		...
		*1) GET THE RIGHT ESTIMATES INTO MEMORY
		...
		if "`modeltype'"=="minadj" & "`variable'"!="agegroup" & "`variable'"!="male" {
			cap estimates use ./output/models/an_covidvsnoncovid_AGESEX_`variable'
		...
		*FOR REST - use the "main" multivariate model
		if "`variable'"=="agegroup" {
			if "`modeltype'"=="minadj" {
				cap estimates use ./output/models/an_covidvsnoncovid_AGESEX_agegroupsex
				if _rc!=0 local noestimatesflag 1
				}
			if "`modeltype'"=="fulladj" {
				cap estimates use ./output/models/an_covidvsnoncovid_MAINFULLYADJMODEL_agegroup_bmicat_noeth
				if _rc!=0 local noestimatesflag 1
				}
			}
		else if "`variable'"=="male" {
			if "`modeltype'"=="minadj" {
				cap estimates use ./output/models/an_covidvsnoncovid_AGESEX_agesplsex
				if _rc!=0 local noestimatesflag 1			
				}
			if "`modeltype'"=="fulladj" {
				cap estimates use ./output/models/an_covidvsnoncovid_MAINFULLYADJMODEL_agespline_bmicat_noeth  
				if _rc!=0 local noestimatesflag 1
				}
			}
		else if "`variable'"=="ethnicity" {
			if "`modeltype'"=="fulladj" {
				cap estimates use ./output/models/an_covidvsnoncovid_MAINFULLYADJMODEL_agespline_bmicat_CCeth  
				if _rc!=0 local noestimatesflag 1
				}
			}			
		else {
...
file open tablecontents using ./output/an_table_covidvsnoncovid.txt, t w replace 
...
scatter graphorder hr if lci>=.15, mcol(black)	msize(small)		///										///
	...
	|| scatter graphorder levelx, m(i) mlab(leveldesc) mlabsize(tiny) mlabcol(gs8) 	///
		xline(1,lp(dash)) 															///
		xscale(log) xlab(0.25 0.5 1 2 4 ) xtitle("Odds Ratio & 95% CI" "for COVID vs non-COVID death") ylab(none) ytitle("")						/// 
		legend(off)  yscale(r(100)) ysize(8) text(98 2 "Higher odds"  "of COVID-19 deaths", size(tiny))  ///
		text(98 0.5 "Higher odds" "of non-COVID deaths", size(tiny))  

graph export ./output/an_table_covidvsnoncovid_PLOT.svg, as(svg) replace
