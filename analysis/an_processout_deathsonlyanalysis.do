...
local endwith "_tab"
	...
	*put the varname and condition to left so that alignment can be checked vs shell
	...
	foreach modeltype of any agesex full {
	
		local noestimatesflag 0 /*reset*/

*CHANGE THE OUTCOME BELOW TO LAST IF BRINGING IN MORE COLS
		if "`modeltype'"=="full" local endwith "_n"
		...
		*FOR REST - use the "main" multivariate model
		if "`variable'"=="agegroup" {
			...
			if "`modeltype'"=="full" {
				cap estimates use ./analysis/output/models/an_deathsonlyanalysis_FULL_agespline_bmicat_noeth  
				if _rc!=0 local noestimatesflag 1
				}
			}
		else if "`variable'"=="ethnicity" {
			if "`modeltype'"=="full" {
				cap estimates use ./analysis/output/models/an_deathsonlyanalysis_FULL_agespline_bmicat_CCeth  
				if _rc!=0 local noestimatesflag 1
				}
			}			
		else {
...
file open tablecontents using ./analysis/output/an_processout_deathsonlyanalysis_TABLE.txt, t w replace 
