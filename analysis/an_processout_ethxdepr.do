...
xtitle("Odds Ratio vs least deprived and 95% CI") title(`titlepref'COVID-19 death) xline(1, lp(dash)) note(" " "open circles = age-sex adjusted (p-interaction = 0.`pint`outcome'agesex')" "solid circles = fully adjusted (p-interaction = 0.`pint`outcome'full')" "(see Bhaskaran et al Lancet Regional Health Europe 2021 for details)", size(tiny)) name(`outcome', replace)
}
