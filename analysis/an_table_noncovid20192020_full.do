...
*CHANGE THE OUTCOME BELOW TO LAST IF BRINGING IN MORE COLS
		...
		*FOR REST - use the "main" multivariate model
		if "`variable'"=="agegroup" {
				cap estimates use ./output/models/an_noncovid_full_`year'_agegroup_bmicat_noeth
				if _rc!=0 local noestimatesflag 1
				}
		else if "`variable'"=="ethnicity" {
				cap estimates use ./output/models/an_noncovid_full_`year'_agespline_bmicat_CCeth  
				if _rc!=0 local noestimatesflag 1
				}			

		else {
				cap estimates use ./output/models/an_noncovid_full_`year'_agespline_bmicat_noeth
				if _rc!=0 local noestimatesflag 1
				}
		...
		*3) Save the estimates for plotting
		if `noestimatesflag'==0{
				...
				cap gen `variable'=.
				testparm i.`variable'
				post HRestimates (string(`year')) ("`variable'") (`i') (`hr') (`lb') (`ub') (r(p))
				drop `variable'
				}	
		} /*failn 1 2 */
...
file open tablecontents using ./output/an_table_noncovid20192020_full.txt, t w replace 
...
graph export ./output/an_table_noncovid20192020_full.svg, as(svg) replace
