...
*an_table_covidvsnoncovid_mortalityORs
*************************************************************************
...
local endwith "_tab"
	...
	*put the varname and condition to left so that alignment can be checked vs shell
	...
	foreach outcome of any COV NONCOV {
	
		local noestimatesflag 0 /*reset*/

*CHANGE THE OUTCOME BELOW TO LAST IF BRINGING IN MORE COLS
		if "`outcome'"=="NONCOV" local endwith "_n"
		...
		*FOR REST - use the "main" multivariate model
		if "`variable'"=="agegroup" {
				cap estimates use ./output/models/an_covidvsnoncovid_mortalityORs_`outcome'_agegroup_bmicat_noeth
				if _rc!=0 local noestimatesflag 1
				}
		else if "`variable'"=="ethnicity" {
				cap estimates use ./output/models/an_covidvsnoncovid_mortalityORs_`outcome'_agespline_bmicat_CCeth  
				if _rc!=0 local noestimatesflag 1
				}			
		else {
				cap estimates use ./output/models/an_covidvsnoncovid_mortalityORs_`outcome'_agespline_bmicat_noeth
				if _rc!=0 local noestimatesflag 1
				}
...
file open tablecontents using ./output/an_table_covidvsnoncovid_mortalityORs.txt, t w replace 
...
graph export ./output/an_table_covidvsnoncovid_mortalityORs.svg, as(svg) replace
