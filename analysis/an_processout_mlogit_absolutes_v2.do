...
graph bar pnoncovdeath pcovdeath , over(variable_level, label(angle(90) labsize(tiny))) over(nameshort, label(nolabels)) nofill legend(label(1 "Pr(non-cov death)") label(2 "Pr(covid death)")) by(agegroup) ylabel(,angle(0))
